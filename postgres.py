...
# Configuration
DB_DSN = "postgresql://mds:mds@localhost:5432/postgres"
...
class PostgresAdapter(Usecases):
    ...
    def _create_tables_if_not_existent(self):
        table_commands = [
            ...
            (
                "listing_id"            INT PRIMARY KEY,
                "area_size_in_square_m" NUMERIC(12, 2),
    ...
    def create_indexes(self):
        index_commands = [
            ...
            # Cities
            """CREATE INDEX IF NOT EXISTS idx_cities_city_name_lower ON "cities" (LOWER("city_name") varchar_pattern_ops);""",
    ...
    def drop_indexes(self):
        index_names = [
            ...
            # "idx_estate_details_has_solar_panels", # Removed: Handled by usecase3 logic or manual drop if needed
...
if __name__ == "__main__":
        ...
        logger.info(f"Bulk imported: {postgres.usecase7_bulk_import()}")  # Import data from the default file
        logger.info(f"Usecase 1: {len(postgres.usecase1_filter_properties())}")
        logger.info(f'Usecase 4: {len(postgres.usecase4_price_analysis()["below_threshold"])}')
        logger.info(f'Usecase 4: {len(postgres.usecase4_price_analysis()["sorted_by_city"])}')
        logger.info(f"Usecase 5: {len(postgres.usecase5_average_price_per_city())}")
        postgres.close()
