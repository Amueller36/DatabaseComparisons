...
class DatabseCaseRun:
    ...
    def from_json(cls, json: dict, run: int):
        ...
        use_case_clean_name = " ".join(use_case_str.split("_")[1:])
        return cls(
    ...
    def __str__(self) -> str:
        return (
            f"DatabseCaseRun(database='{self.database}', "
            f"timestamp={self.timestamp.isoformat()}, "
            f"usecase={self.usecase} ('{self.usecase_full_name}'), "
            f"execution_time={self.execution_time}s ({self.execution_time_ms}ms), "
            f"avg_cpu={self.avg_cpu_percent}%, peak_cpu={self.peak_cpu_percent}%, "
            f"avg_mem={self.avg_mem_mb}MB, peak_mem={self.peak_mem_mb}MB)"
        )
...
def parse_filename(file_name: str):
    ...
    return fileparts[0], datetime.strptime(fileparts[1], "%d-%m-%Y"), int(fileparts[2].replace("Run", ""))
